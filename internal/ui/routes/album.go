...
func Album(albumId string) *router.Response {
	...
	isAlbumFavourited := signals.NewStatefulSignal(false)
					...
					HStack(
						Button().
							...
							ConnectConstruct(func(b *gtk.Button) {
								...
								isAlbumFavourited.Notify(func(oldValue bool) bool {
									return slices.Contains(favList, albumId)
								})

								weakRef := tracking.NewWeakRef(&b.Object)
								isAlbumFavourited.On(func(value bool) bool {
							...
							ConnectClicked(func(b gtk.Button) {
								...
								isAlbumFavourited.Notify(func(oldValue bool) bool {
									if oldValue {
										err := tidal.V1.Favourites.RemoveAlbum(context.Background(), secrets.UserID(), albumId)
										if err != nil {
											albumLogger.Error("error while removing album from favourites", "error", err)
											return oldValue
										}
									} else {
										err := tidal.V1.Favourites.AddAlbum(context.Background(), secrets.UserID(), albumId)
										if err != nil {
											albumLogger.Error("error while adding album to favourites", "error", err)
											return oldValue
										}
									}
